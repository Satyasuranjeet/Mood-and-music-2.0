...
console.log(import.meta.env.VITE_GOOGLE_GEN_AI_KEY);
...
const MoodMusicChatbot = ({ emotion }) => {
  ...
  useEffect(() => {
    if (emotion && emotion !== detectedEmotion) {
      ...
      // Only add a message if it's the first time detecting this emotion
      // or if we're switching from a previously detected emotion
  ...
  // Handle initial emotion detection - just ask for confirmation
  ...
  // Process mood confirmation - to be called after user confirms their mood
  const handleMoodConfirmation = async (confirmed, emotion) => {
    ...
    if (confirmed) {
      ...
      try {
        // Use Google Generative AI for a more personalized response
  ...
  // Process song data from the API response
  ...
  // Search for songs using the actual API
  ...
  // Handle user input with Gemini API integration
  const handleSubmit = async (e) => {
    ...
    if (detectedEmotion && !moodConfirmed) {
      ...
      if (isPositive) {
        ...
        // Wait for user response to music offer
        return;
      } else if (isNegative) {
        handleMoodConfirmation(false, detectedEmotion);
        return;
      } else {
        // If we can't clearly tell yes/no, try again
    ...
    for (const [scenario, data] of Object.entries(complexScenarios)) {
      if (data.keywords.some(keyword => userMessage.toLowerCase().includes(keyword))) {
        ...
        try {
          // Use Google Generative AI for personalized response
          const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
          ...
          const thoughtfulResponses = [
            "Music has an incredible way of helping us process our emotions during difficult times. I've found some tracks that might resonate with what you're going through.",
            "When we're experiencing intense emotions, music can be a powerful outlet. I've selected some songs that might speak to your current situation.",
            "Sometimes the right song can express feelings we can't put into words. Let me share some music that might complement how you're feeling right now."
          ];
          
          addMessage('bot', thoughtfulResponses[Math.floor(Math.random() * thoughtfulResponses.length)]);
        }
    ...
    if (!scenarioDetected) {
      // Use Gemini AI to analyze user message
      setIsLoading(true);
      ...
      try {
        ...
        // Ask for confirmation of emotion
        addMessage('bot', `Just to confirm, would you say you're feeling ${newEmotion.toLowerCase()}?`);
        setMoodConfirmed(false);
      } catch (error) {
        ...
        if (userMessage.toLowerCase().includes("happy") || userMessage.toLowerCase().includes("joy")) {
          newEmotion = "Happy";
          ...
          newEmotion = "Angry";
        } else if (userMessage.toLowerCase().includes("scared") || userMessage.toLowerCase().includes("fear")) {
          newEmotion = "Fear";
        }
        
        setDetectedEmotion(newEmotion);
        addMessage('bot', `It sounds like you might be feeling ${newEmotion.toLowerCase()}. Is that accurate?`);
        setMoodConfirmed(false);
      } finally {
        setIsLoading(false);
      }
  ...
  const playSong = (song) => {
    ...
    // Actually play the song if we have a reference to the audio element
    if (audioRef.current) {
  ...
  {showPlayer && currentSong && (
    ...
    >
      ...
      <audio 
        ...
        onEnded={() => {
          // When one song ends, automatically play next song if available
          const currentIndex = searchResults.findIndex(song => song.id === currentSong.id);
          if (currentIndex >= 0 && currentIndex < searchResults.length - 1) {
            playSong(searchResults[currentIndex + 1]);
          }
